<template>
  ...
  <x-article>
    <main slot='main'>
      <ul v-if='editDetailInfo.t_qa_info.length'  class='p-t-15'>
        
        <li v-for='(item,index) in editDetailInfo.t_qa_info' :key='index'>
          ...
          <router-link v-else  :to='"edit/"+index' :key='index' class="f-b-c h-90 b-2-eee b-no-lr b-no-t">
    ...
    <!-- <div slot='footer' class="p-r f-c f-1 b-r-45" > -->
      ...
      <section class='p-a h-100 w-per-100 bg-color-ccc translateY-per-100-reverse zezhao-footer'>
        <span v-if='editDetailInfo.t_qa_info.length<10' @click='isEnableButton && $router.push("edit/add")'>
          <Icon type='icon-add'  class='p-a-xr-yt b-r-circle translateY-40-reverse' :class='isEnableButton?"add-shadow f-s-90 c-1C71B7 ":"f-s-90 c-ccc "' />
        </span>
...
export default {
  ...
  methods:{
    ...mapActions('enterDetail',['requestVision','requestDeleteVision']),
    ...
    async submitAudit(){
      ...
      this.editDetailInfo.t_qa_info.forEach(item=>{
